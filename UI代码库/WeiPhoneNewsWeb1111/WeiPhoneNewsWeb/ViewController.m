...
- (void)requestFinished:(ASIHTTPRequest *)request{
    ...
    if (self.page==0&&self.newsTitleArrayN.count==75) {
        ...
        [self sortedDate];
    }else if(self.newsTitleArrayN.count>75&&self.newsTitleArrayN.count%75==0) {
...
//排序方法
-(void)sortedDate{
    self.sortedDateArray=[self.newsArrayN sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
...
{
    static NSString *cellStr1 = @"cell";//重用标示，，，单元格的名称。索引机制
    //重用方式获取单元格对象
...
//        NSString *news=[self.newsTitleArrayN objectAtIndex:indexPath.row-1];
//        cell.newsLab.text=news;
//        NSString *titleFrom=[self.webArrayN objectAtIndex:indexPath.row-1];
//        self.wordArray=[titleFrom componentsSeparatedByString:@"/"];//切割网页
//        NSString *title=[self.wordArray objectAtIndex:3];
//        NSString *num=[self.lookNoArrayN objectAtIndex:indexPath.row-1];
//        cell.titleFromLab.text=[NSString stringWithFormat:@"%@新闻(%@次浏览)",title,num];
//        NSString *date=[self.dateArrayN objectAtIndex:(indexPath.row-1)*2];
//        cell.dateLab.text=[NSString stringWithFormat:@"%@",date];
