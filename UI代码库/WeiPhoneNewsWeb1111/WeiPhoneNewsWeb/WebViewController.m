...
//    self.scrollView.contentSize = CGSizeMake(320, self.scrollView.frame.size.height);//设置页面的长度
    ...
    NSString *idStr = [self.willToUrl substringWithRange:cutUrlStr];
    NSLog(@"截取的字符串----------%@",idStr);
    ...
    moreImage.contentMode = UIViewContentModeScaleAspectFill ;
...
//    NSString *coStr = [NSString stringWithFormat:@"更多评论（%d）",self.commentArray.count];
    ...
    //剩下的评论
    UILabel *moreComST = [[UILabel alloc]initWithFrame:CGRectMake(180, 10, 80, 30)];
    moreComST.textColor = [UIColor colorWithRed:99.0f/255.0f green:99.0f/255.0f blue:99.0f/255.0f alpha:1.0];
    ...
    [self.moreCommentV addSubview:moreComST];
}
...
- (float )tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    // 這裏返回需要的高度
//    if (indexPath.row) {
//    return 133+self.cellAddHight;
//    }
    return 140+self.cellAddHight;
}
